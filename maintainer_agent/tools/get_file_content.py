...
def get_files_content(
    ...
    """
    Gets the content and metadata of multiple files from a GitHub repository and automatically
    stores them in the session state.
    ...
    try:
        ...
        # Fetch each file sequentially
        for file_path in file_paths:
            try:
                print(f"[GET_FILES_CONTENT] Fetching {file_path}...")
                ...
                # Check if it's an error response
                if content.startswith("Error fetching file:"):
                    failed_files[file_path] = content
                    print(f"[GET_FILES_CONTENT] Failed to fetch {file_path}: {content}")
                    continue
                ...
                print(f"[GET_FILES_CONTENT] Successfully fetched {file_path} ({metadata['size']} bytes)")
                
            except Exception as file_error:
                failed_files[file_path] = str(file_error)
                print(f"[GET_FILES_CONTENT] Error fetching {file_path}: {file_error}")
        
        # Determine overall status
        if len(successful_files) == len(file_paths):
            status = 'success'
            message = f"Successfully fetched all {len(file_paths)} files"
